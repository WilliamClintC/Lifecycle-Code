...
def get_output_directory():
    """Return the output directory for all files."""
    output_dir = r"C:\Users\clint\Desktop\Lifecycle Code\data\pdf_links\individual"
...
def navigate_to_page(driver, query, page_num):
    ...
    if page_num > 1:
        ...
        if "start=" not in current_url:
            ...
            debug_screenshot = os.path.join(get_output_directory(), f"page{page_num}_debug.png")
...
def save_to_file(results, query, filename=None):
    """Save search results to a CSV file in the specified directory."""
    # Define the output directory
    output_dir = get_output_directory()
    
    # If no filename is provided, create one based on the query
    if not filename:
        # Clean the query to make it suitable for a filename
        clean_query = re.sub(r'[^\w\s-]', '', query.replace(':', '_').replace('"', ''))
        clean_query = re.sub(r'\s+', '_', clean_query)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"dorking_links_{clean_query}_{timestamp}.csv"
...
def main():
    ...
    for query_index, query in enumerate(search_queries, 1):
        print(f"\n{'='*80}")
        print(f"Processing query {query_index} of {len(search_queries)}: {query}")
        print(f"{'='*80}\n")
        ...
        try:
            ...
            for page_num in range(1, 5):
                ...
                # Take a screenshot for verification and save to output directory
                screenshot_path = os.path.join(get_output_directory(), f"query{query_index}_page{page_num}_results.png")
                driver.save_screenshot(screenshot_path)
                print(f"Saved screenshot of page {page_num} results to: {screenshot_path}")
            ...
            if all_results:
                ...
                filename = save_to_file(all_results, query)
                print(f"Opening {filename}...")
                os.system(f'start "" "{filename}"')  # Improved command to handle paths with spaces
        except Exception as e:
            import traceback
            print(f"An error occurred with query {query_index}: {str(e)}")
            print(traceback.format_exc())
        finally:
            print(f"Closing browser for query {query_index}...")
            driver.quit()
